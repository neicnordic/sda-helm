...
helm install sda charts/sda-svc -f dev_tools/config/posix.yaml \
    --set global.broker.vhost=/sda,global.c4gh.passphrase="$C4GH_PASSPHRASE",global.db.passIngest="$DB_IN_PASS",global.db.passOutgest="$DB_OUT_PASS",global.schemaType=isolated,intercept.deploy=$INTERCEPT
