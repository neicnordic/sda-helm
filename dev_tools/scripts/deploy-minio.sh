...
MINIO_ACCESS=$(grep s3_archive_access_key sda-deploy-init/config/trace.yml | awk {'print $2'} | sed --expression 's/\"//g')
MINIO_SECRET=$(grep s3_archive_secret_key sda-deploy-init/config/trace.yml | awk {'print $2'} | sed --expression 's/\"//g')
